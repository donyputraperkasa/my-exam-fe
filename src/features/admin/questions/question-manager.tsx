...
export function QuestionManager() {
  ...
  const filteredQuestions = useMemo(() => filterQuestions(questions, selectedPackageId, searchQuery), [questions, searchQuery, selectedPackageId]);
  ...
  async function refreshQuestions() {
    const token = getToken();
    if (token) setQuestions((await fetchAdminQuestions({ gradeId: filterGradeId, subjectId: filterSubjectId }, token)).data);
  }
  ...
  function handleEdit(question: Question) {
    setEditingId(question.id); setForm(mapQuestionToForm(question)); setIsFormOpen(true);
  }
  function handleCreateQuestion() {
    resetForm(); setError(""); setIsFormOpen(true);
  }
  function handleCloseForm() {
    resetForm(); setError(""); setIsFormOpen(false);
  }
  ...
  return (
    <section className="grid w-full grid-cols-1 gap-5">
      <QuestionToolbar onCreate={handleCreateQuestion} onSearch={setSearchQuery} searchQuery={searchQuery} />
      <QuestionPackageCards onSelect={setSelectedPackageId} packages={packages} selectedId={selectedPackageId} />
      <QuestionPanel filterGradeId={filterGradeId} filterSubjectId={filterSubjectId} grades={grades} loading={loading} onDelete={handleDelete} onEdit={handleEdit} onFilterGrade={setFilterGradeId} onFilterSubject={setFilterSubjectId} questions={filteredQuestions} subjects={subjects} />
      {isFormOpen ? <QuestionFormModal editingId={editingId} error={error} form={form} grades={grades} onChange={handleFormChange} onClose={handleCloseForm} onImageUpload={handleImageUpload} onSubmit={handleSubmit} packages={packages} subjects={subjects} /> : null}
    </section>
  );
}
