...
export function PaymentProofModal({ item, onClose }: PaymentProofModalProps) {
  ...
  return (
    ...
    >
      <section className="max-h-[calc(100dvh-2rem)] w-full max-w-3xl overflow-y-auto rounded-2xl border border-violet-100 bg-white p-4 shadow-2xl sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="break-all text-xs font-bold text-muted">{item.user?.name} · {item.invoiceCode}</p>
            </div>
          </div>
        ...
        <div className="mt-5 flex min-h-56 w-full items-center justify-center overflow-hidden rounded-xl border border-violet-100 bg-background sm:h-[min(58vh,620px)]">
          {/* URL bukti bersifat dinamis, sehingga elemen gambar native lebih stabil di browser mobile. */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={item.proofImageUrl}
            alt={`Bukti transfer ${item.invoiceCode}`}
